...
-- DuckDB-dialect changes (no check added, removed, weakened, or reworded):
--   * the `${catalog}.${wh_db}_${scale_factor}.` schema prefix stripped -> bare table
--     names (duckrun sets the schema on the connection);
--   * Spark `! IsCurrent` -> `NOT IsCurrent` (4x);
...
-- The DImessages-driven checks read the real per-batch log the orchestrator writes
-- (batch_initial/batch_complete/batch_validation/visibility_1/visibility_2/audit_alerts).
select *
from (
    ...
    select 'DimAccount EndDate', NULL, case
        when (
          ...
          select count(*)
          from DimAccount a
            join DimAccount b on a.AccountID = b.AccountID
            ...
            case
              when (
                ...
                from (
                  ...
                  join DimAccount da ON da.batchid = aud.batchid
                  AND (
                    (
            ...
            case
              when (
                ...
                from (
                  ...
                  join DimCustomer dc ON dc.batchid = aud.batchid
                  AND (
                    (
    ...
    from (
        select distinct BatchID, case
            when (
              select sum(cast(MessageData as bigint))
              from dimessages
    ...
    select 'DimCustomer LocalTaxRate', NULL, case
        when (
          ...
          select count(*)
          from DimCustomer c
            join taxrate t on c.LocalTaxRateDesc = t.TX_NAME
    ...
    select 'DimCustomer NationalTaxRate', NULL, case
        when (
          ...
          select count(*)
          from DimCustomer c
            join taxrate t on c.NationalTaxRateDesc = t.TX_NAME
    ...
    select 'DimSecurity EndDate', NULL, case
        when (
          ...
          select count(*)
          from DimSecurity a
            join DimSecurity b on a.Symbol = b.Symbol
            and a.EndDate = b.EffectiveDate
        ) + (
          select count(*)
          from DimSecurity
            ...
            case
              when (
                ...
                from (
                  ...
                  join DimSecurity dc ON dc.batchid = aud.batchid
                  AND (
                    (
                      dc.EffectiveDate < aud.date
                      AND Attribute = 'FirstDay'
    ...
    from (
        select distinct BatchID, case
            when --added sum(cast(MessageData as bigint))
            cast(
              (
                select sum(cast(MessageData as bigint))
                from dimessages
    ...
    select 'DimCompany EndDate', NULL, case
        when (
          ...
          select count(*)
          from DimCompany a
            join DimCompany b on a.CompanyID = b.CompanyID
            and a.EndDate = b.EffectiveDate
        ) + (
          select count(*)
          from DimCompany
            ...
            case
              when (
                ...
                from (
                  ...
                  join DimCompany dc ON dc.batchid = aud.batchid
                  AND (
                    (
                      dc.EffectiveDate < aud.date
                      AND Attribute = 'FirstDay'
    ...
    select 'DimCompany distinct names', NULL, case
        when (
          select count(*)
          from DimCompany a
            join DimCompany b on a.Name = b.Name
        ...
        select distinct BatchID, (
            case
              when (
                select count(*)
                from Prospect p
                  join DimDate on SK_DateId = P.SK_RecordDateID
                  join Audit _A on DataSet = 'Batch'
                  and Attribute = 'FirstDay'
                  and DateValue < Date
    ...
    from (
        select distinct BatchID, case
            when (
              select cast(sum(cast(MessageData as bigint)) as bigint)
              from dimessages
              where MessageSource = 'FactWatches'
                and MessageType = 'Validation'
                and MessageText = 'Row count'
    ...
    select 'FactWatches SK_CustomerID', NULL, case
        when (
          ...
          from FactWatches a
            ...
            join DimCustomer c on a.SK_CustomerID = c.SK_CustomerID
            and c.EffectiveDate <= _d.DateValue --(select DateValue from DimDate where SK_DateID = a.SK_DateID_DatePlaced)
            and _d.DateValue <= c.EndDate
        ) --(select DateValue from DimDate where SK_DateID = a.SK_DateID_DatePlaced) <= c.EndDate )
        then 'OK'
        else 'Bad join'
      end, 'All SK_CustomerIDs match a DimCustomer record with a valid date range'
    union all
    ...
    from (
        select distinct BatchID, (
            case
              when --added sum(cast(MessageData as bigint)) and sum(value)
              cast(
                (
                  select sum(cast(MessageData as bigint))
                  from dimessages
    ...
    select 'FactMarketHistory SK_CompanyID', NULL, case
        when (
          ...
          select count(*)
          from FactMarketHistory a
            join DimDate _d on _d.SK_DateID = a.SK_DateID
            join DimCompany c on a.SK_CompanyID = c.SK_CompanyID
            and c.EffectiveDate <= _d.DateValue
            and _d.DateValue <= c.EndDate
        ) --(select DateValue from DimDate where SK_DateID = a.SK_DateID) and (select DateValue from DimDate where SK_DateID = a.SK_DateID) <= c.EndDate)
        then 'OK'
        else 'Bad join'
      end, 'All SK_CompanyIDs match a DimCompany record with a valid date range'
    union all
                ...
                (
                  ...
                  where m.BatchID = a.BatchID
                    and (
                      ...
                      select first(Date) - INTERVAL 1 DAY
                      from Audit
                      where DataSet = 'Batch'
    ...
    select 'FactHoldings SK_CustomerID', NULL, case
        when (
          ...
          from FactHoldings a
            join DimDate _d on _d.SK_DateID = a.SK_DateID
            join DimCustomer c on a.SK_CustomerID = c.SK_CustomerID
            and c.EffectiveDate <= _d.DateValue --(select DateValue from DimDate where SK_DateID = a.SK_DateID)
        ) then 'OK'
        else 'Bad join'
      end, 'All SK_CustomerIDs match a DimCustomer record with a valid date range'
    union all
    ...
    select 'FactHoldings CurrentTradeID', NULL, case
        when (
          ...
          select count(*)
          from FactHoldings a
            join DimTrade t on a.CurrentTradeID = t.TradeID
    ...
    from (
        ...
        from Audit a full
          ...
          and m1.MessageText = 'Row count'
          and m1.MessageType = 'Validation'
          join dimessages m2 on m2.BatchID = a.BatchID
          and m2.MessageSource = m.MessageSource
    ...
    from (
        ...
        from dimessages m1
          join dimessages m2 on m2.MessageType IN ('Visibility_1', 'Visibility_2')
          and m2.MessageText = 'Row count'
    ...
    from (
        ...
        from dimessages m1
          join dimessages m2 on m2.MessageType = 'Visibility_1'
          and m2.MessageText = 'Row count joined'
          and m2.MessageSource = m1.MessageSource
          and m2.MessageDateAndTime = m1.MessageDateAndTime
