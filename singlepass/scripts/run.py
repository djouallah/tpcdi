"""Drive the single-pass TPC-DI load on duckrun: generate -> dbt run -> dbt test.

One `dbt run` reads Batch1 (historical) plus Batch2/3 (incremental CDC) together and
produces the complete SCD2 warehouse end-state. This is the single-pass project's own
runner; the sequential 3-batch mode is a separate, self-contained project in
`sequential/` with its own `scripts/run.py`.

Local:
    python singlepass/scripts/run.py --sf 3
OneLake (Delta output to a Fabric Lakehouse):
    WAREHOUSE_PATH=abfss://.../Tables ONELAKE_TOKEN=... \\
        python singlepass/scripts/run.py --sf 3 --target onelake
"""
...
PROJ = os.path.dirname(HERE)  # the singlepass/ dbt project dir
